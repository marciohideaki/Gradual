...
namespace Gradual.OMS.InvXX.Fundos.Teste
{
    public partial class frmImportacaoANBIMA : Form
    {
        ...
        private void lDataInicial_Click(object sender, EventArgs e)
        {
        ...
        private void btnImportarTodosFundosANBIMA_Click(object sender, EventArgs e)
        {
            var lServico = new ImportacaoFundosTodos();

            try
            {
                lServico.ThreadImportacaoAnbima(this, true);

                MessageBox.Show("Importação dos fundos dos arquivos da anbima efetuada com sucesso" , "Importação de todos os fundos dos arquivos da anbima", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message ,"Erro na importação de todos os fundos", MessageBoxButtons.OK, MessageBoxIcon.Error);
