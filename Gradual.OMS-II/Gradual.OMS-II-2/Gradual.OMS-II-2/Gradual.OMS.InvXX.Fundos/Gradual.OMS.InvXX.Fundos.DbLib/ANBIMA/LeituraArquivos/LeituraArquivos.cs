...
namespace Gradual.OMS.InvXX.Fundos.DbLib.ANBIMA.LeituraArquivos
{
    public class LeituraArquivos
    {
        ...
        public List<FundosInfo> LeArquivoCadFundos()
        {
            List<FundosInfo> lRetorno = new List<FundosInfo>();

            try
            {
                string lArquivo              = GetNomeCorretoArquivo(ListaArquivos["Fundos"]);
                ...
                foreach (string linha in Linhas)
                {
                    if (linha != "\n")
                    {
                        gLogger.InfoFormat("Linha de Cadastro de Fundos encontrada [{0}].", linha);
                        lRetorno.Add(RetornaFundos(linha));
                    }
                }
            }
            catch (Exception ex)
            {
                gLogger.ErrorFormat("Erro encontrado no método LeArquivoCadFundos - [{0}]", ex.StackTrace);
            }

            return lRetorno;
        }

        public List<ANBIMAFundosInfo> LeArquivoCadastroFundos()
        {
            var lRetorno = new List<ANBIMAFundosInfo>();
            ...
            try
            {
                ...
                foreach (string linha in Linhas)
                {
                    ...
                    {
                        gLogger.InfoFormat("Linha de Cadastro de Fundos encontrada [{0}].", linha);
                        lRetorno.Add(RetornaFundosAnbima(linha));
                    }
                }
            }
            catch (Exception ex)
