...
namespace GradualForm
{
	...
	public class StickyWindow : System.Windows.Forms.NativeWindow
	{
		...
		private bool DefaultMsgProcessor ( ref Message m )
		{
			switch ( m.Msg )
			{
				case Win32Imports.WM.WM_NCLBUTTONDOWN:
				{
					originalForm.Activate();
					mousePoint.X = (short)Win32Imports.Bit.LoWord ( (int)m.LParam );
					mousePoint.Y = (short)Win32Imports.Bit.HiWord ( (int)m.LParam );
		...
		private bool OnNCLButtonDown ( int iHitTest, Point point )
		{
			...
			switch ( iHitTest )
			{
				case Win32Imports.HT.HTCAPTION:
				{	// request for move
					if ( stickOnMove )
					{
		...
		private bool ResizeMsgProcessor(ref Message m)
		{
			...
			switch ( m.Msg )
			{
				case Win32Imports.WM.WM_LBUTTONUP:
				{	// ok, resize finished !!!
					EndResize();
					break;
				}
				case Win32Imports.WM.WM_MOUSEMOVE:
				{
					mousePoint.X = (short)Win32Imports.Bit.LoWord ( (int)m.LParam );
					mousePoint.Y = (short)Win32Imports.Bit.HiWord ( (int)m.LParam );
					Resize( mousePoint );
				...
				case Win32Imports.WM.WM_KEYDOWN:
				{
					if ( (int)m.WParam == Win32Imports.VK.VK_ESCAPE )
					{
