...
namespace Gradual.OMS.RelatoriosFinanc
{
    public static class Extensions
    {
        public static Int32 DBToInt32(this object pObject)
        {
            ...
            if (pObject == null || pObject == DBNull.Value) lReturn = 0;

            string lValorEntrada = pObject.DBToString();

            if (lValorEntrada.Contains(".") || lValorEntrada.Contains(","))
            {
                int lIndexOf = lValorEntrada.Contains(".") ? lValorEntrada.IndexOf('.') : lValorEntrada.IndexOf(',');

                lValorEntrada = lValorEntrada.Remove(lIndexOf);
            }

            if (!int.TryParse(lValorEntrada.Replace(".", ","), out lReturn)) lReturn = 0;
        ...
        public static decimal DBToDecimal(this object pObject)
        {
            ...
            if (Decimal.TryParse(pObject.DBToString().Replace(".", ","), System.Globalization.NumberStyles.Any, new System.Globalization.CultureInfo("pt-BR"), out lReturn))
            {
                return lReturn;
            }
            else
        ...
        public static double DBToDouble(this object pObject)
        {
            ...
            double.TryParse(pObject.DBToString().Replace(".", ","), System.Globalization.NumberStyles.Any, new System.Globalization.CultureInfo("pt-BR"), out lRetorno);
        ...
        public static string DBToDateTimeString(this object pObject)
        {
            if (pObject == null || pObject == DBNull.Value) return string.Empty;

            var lDataSaida = default(DateTime);

            if (DateTime.TryParse(pObject.DBToString(), out lDataSaida) && lDataSaida != DateTime.MinValue)
                return lDataSaida.ToString("MM/dd/yyyy");
        ...
        public static DateTime DBToDateTime(this object pObject)
        {
            ...
            if (pObject is DateTime)
            {
                return (DateTime)pObject;
            }
            else if (DateTime.TryParse(pObject.ToString(), System.Globalization.CultureInfo.GetCultureInfo("pt-BR"), DateTimeStyles.None, out lReturn))
            {
                return lReturn;
            }
            else
