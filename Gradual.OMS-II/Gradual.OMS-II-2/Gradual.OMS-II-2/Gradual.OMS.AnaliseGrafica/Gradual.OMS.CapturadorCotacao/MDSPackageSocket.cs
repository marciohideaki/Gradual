...
namespace Gradual.OMS.CapturadorCotacao
{
    ...
    public class MDSPackageSocket
    {
        ...
        public const string TIPOMSG_UNITARIO = "PU";
        public const string TIPOMSG_COMPOSICAOINDICE = "CI";

        public string [] TIPOS_MSG = { "NE", "CB", "FE", "AB", "AJ", "PU", "CI" };

        public string IpAddr { get; set; }
        public int Port { get; set; }

        public DateTime LastPacket { get; set; }
        public event MDSMessageReceivedHandler OnMensagemMdsReceived;
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
            ...
            try
            {
                lock (this)
                {
                    ...
                    {
                        int msgTypeIndex = IndexOfAny(Message, TIPOS_MSG, 8);
                        if (msgTypeIndex < 0 || msgTypeIndex > 31)
                            msgTypeIndex = 8;

                        Instrumento = Message.Substring(0, msgTypeIndex);
                        Message = Message.Remove(0, msgTypeIndex);
        ...
        public int IndexOfAny(string test, string[] values, int startindex)
        {
            int first = -1;
