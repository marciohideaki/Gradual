    ...
    public class MDSPackageSocket
    {
        ...
        private const string A4 = "A4";
         ...
         private string IpAddr{
            get{
                return ConfigurationSettings.AppSettings["ASConnMDSIp"].ToString();
            }
        }

        private string Port{
            get{
                return ConfigurationSettings.AppSettings["ASConnMDSPort"].ToString();
            }
        }
        ...
        public void OpenConnection()
        {
            ...
            {
                WriteEventLog(string.Format("{0}{1}", "OpenConnection: ", se.Message));
                throw (se);
            }
        ...
        public void SendData(string msg, bool SendLenght)
        {
            ...
            {
                WriteEventLog(string.Format("{0}{1}", "SendData: ", ex.Message));
                throw ex;
            }
        ...
        private string ReceiveData(int BytesReceive, int BufferSize, Socket SocketData)
        {
            ...
            {
                WriteEventLog(string.Format("{0}{1}", "ReceiveData: ", ex.Message));
                throw ex;
            }
        }
        ...
        private void TheadWaitOne()
        {
            ...
            {
                WriteEventLog(string.Format("{0}{1}", "TheadWaitOne: ", se.Message));
                throw (se);
            }
        }
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
            ...
            try
            {
                ...
                if (se.ErrorCode == (int)(SocketError.ConnectionReset))
                {
                    // Falha de conexão
                    this.MDSSocketClose(socketData);
                    WriteEventLog(string.Format("{0}{1}", "OnDataReceived: ", "Falha de conexão"));
                }
            }
            catch (Exception ex){
                WriteEventLog(string.Format("{0}{1}", "OnDataReceived: ", ex.Message));
                throw ex;
            }
        }
            ...
            try
            {
                switch (recv.ToString().Substring(0, 2))
                {
                    ...
                    case StopSimples:
                        ...
                        // Construção similar a :
                        // int *p = (int *) (malloc(sizeof(int));
                        _SS_StopSimplesResposta = (SS_StopSimplesResposta)(Marshal.PtrToStructure(pBufMessage, typeof(SS_StopSimplesResposta)));
