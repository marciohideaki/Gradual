...
namespace Orbite.RV.Sistemas.Integracao.BVMF
{
    public class ServicoIntegracaoBVMFArquivos : IServicoIntegracaoBVMFArquivos
    {
        ...
        public DataSet InterpretarArquivo(string arquivo)
        {
            ...
            DataSet ds = this.InterpretarArquivo(file, new FileInfo(arquivo).Name);
            file.Close();
            return ds;
        }

        public DataSet InterpretarArquivo(Stream arquivo, string nomeArquivo)
        {
            // Inicializa
            ...
            // Infere o layout
            ArquivoBVMFInfo arquivoBVMF = this.InferirLayout(servicoPersistenciaLayouts.ListarLayouts(), arquivo, nomeArquivo);
        ...
        public ArquivoBVMFInfo InferirLayout(List<LayoutBVMFInfo> layouts, string arquivo)
        {
            ...
            // Repassa a chamada
            ArquivoBVMFInfo arquivoBVMF = InferirLayout(layouts, stream, new FileInfo(arquivo).Name);
            stream.Close();
            if (arquivoBVMF != null)
        ...
        public ArquivoBVMFInfo InferirLayout(List<LayoutBVMFInfo> layouts, Stream stream, string nomeArquivo)
        {
            ...
            foreach (var conversor in conversores)
            {
                ...
                // Pede a inferencia
                arquivoInferido = conversorInstancia.InferirLayout(layouts2, stream, nomeArquivo);
        ...
        public List<ArquivoBVMFInfo> ListarDiretorio()
        {
            // Le arquivo default do config
            ServicoIntegracaoBVMFArquivosConfig config = GerenciadorConfig.ReceberConfig<ServicoIntegracaoBVMFArquivosConfig>();
            
            // Retorna
            return ListarDiretorio(config.DiretorioDefault);
        }
