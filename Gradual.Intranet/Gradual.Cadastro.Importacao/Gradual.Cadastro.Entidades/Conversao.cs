...
namespace Gradual.Generico.Geral
{
    public class Conversao
    {
        ...
        public static DateTime? ToDateTime(object valor)
        {
            ...
            if (valor != null && DateTime.TryParse(valor.ToString(), out retorno))
                return retorno;
            else
                return null;
        }
        ...
        public static int RecuperarDadosAgencia(int pCdBanco, int pCdAgencia, bool pRequisicaoDigito)
        {
            var lRetorno = default(int);
            var lCdAgencia = pCdAgencia.ToString();

            switch (pCdBanco)
            {
                default:
                    lCdAgencia = lCdAgencia.PadLeft(7, '0');

                    if (!pRequisicaoDigito)
                        lRetorno = ToInt32(lCdAgencia.Substring(0, 5));
                    else
                        lRetorno = ToInt32(lCdAgencia.Substring(5, 2));
                        break;
            }
