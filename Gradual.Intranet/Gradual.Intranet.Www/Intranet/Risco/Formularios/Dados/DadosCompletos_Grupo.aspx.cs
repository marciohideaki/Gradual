...
namespace Gradual.Intranet.Www.Intranet.Risco.Formularios.Dados
{
    public partial class DadosCompletos_Grupo : PaginaBaseAutenticada
    {
        ...
        private List<GrupoInfo> SessionUltimoResultadoDeBusca
        {
            get
            {
                if (null != this.Session["UltimoResultadoDeBuscaDeGruposDeRisco"] && this.Session["UltimoResultadoDeBuscaDeGruposDeRisco"] is List<GrupoInfo>)
                    return (List<GrupoInfo>)this.Session["UltimoResultadoDeBuscaDeGruposDeRisco"];

                return new List<GrupoInfo>();
            }
            set
            {
                this.Session["UltimoResultadoDeBuscaDeGruposDeRisco"] = value;
            }
        }
        ...
        private string ResponderBuscarItensParaListagemSimples()
        {
            ...
            var lResponse = base.ServicoRegrasRisco.ListarGrupos(new ListarGruposRequest() { FiltroTipoGrupo = EnumRiscoRegra.TipoGrupo.GrupoDeRisco });
        ...
        private string ResponderExcluir()
        {
            ...
            {
                var lGrupoSelecionado = base.ServicoRegrasRisco.ListarGrupoItens(new ListarGrupoItemRequest() { FiltroIdGrupo = lId.DBToInt32() });
