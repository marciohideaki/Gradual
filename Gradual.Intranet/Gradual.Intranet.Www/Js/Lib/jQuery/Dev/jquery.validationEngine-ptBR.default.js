    ...
    $.validationEngineLanguage = {
        newLang: function () {
            $.validationEngineLanguage.allRules = { "required": {    			// Add your regex rules here, you can take telephone as an example
                ...
                "celular_sp": {
                    "regex": "/[0-9]{5}-[0-9]{4}/",
                    "alertText": "Telefone inv&aacute;lido; formato: 99999-9999"
                },
                "cep": {
                    "regex": "/[0-9]{5}-[0-9]{3}/",
                ...
                //CPF e CNPJ
                "validatecpfcnpj": {
                    "nname": "validatecpfcnpj",
                    "alertText": "CPF ou CNPJ inv&aacute;lido!"
                },

                //CPF e CNPJ
                "validatelengthh": {
                    "nname": "validatelengthh",
                    "alertText": "Campo deve ter de 8 a 15 caracteres"
                },
                ...
                "ValorLimiteBovespa": {
                    "nname": "VerificarValorLimiteBovespa",
                    "alertText": "Valor inv&aacute;lido"
                },

                "DataLimiteBovespa": {
                    "nname": "VerificarDataLimiteBovespa",
                ...
                "validateuppercaracter": {
                    "nname": "validateuppercaracter",
                    "alertText": "Campo deve ter pelo menos um caracter mai&uacutesculo"
                },
                
                "ValidatePasswordField": {
                    "nname": "ValidatePasswordField",
                    //"alertText": "Campo deve ter pelo menos um caracter min&uacutesculo"
                }
...
function ValidatePasswordField(field, rules, i, options) 
{
    var lCaller = $(field).val();
...
function validatelengthh(caller)
{
    var l = $(caller).val();

    return !(l.length >= 8 && l.length <= 15);
}

function validateuppercaracter(caller) 
{
    var lCaller = $(caller).val();

    var goodChars = "ABCDEFGHIJKLMNOPQRSTUXYWVZ";

    var lRetorno = goodChars.indexOf(lCaller) == -1  ? false : true ;
...
function validatelowercaracter(caller) 
{
    var lCaller = $(caller).val();
...
{
 //var the
    var theCPF   = $(caller).val().replace('.','','gi').replace('.','','gi').replace('.','','gi').replace('-','','gi').replace('/','','gi');
    var checkStr = $(caller).val().replace('.','','gi').replace('.','','gi').replace('.','','gi').replace('-','','gi').replace('/','','gi');
    var checkOK  = "0123456789"; 
    var allValid = true;
    var allNum = "";
