create or replace procedure prc_SelecionaComboSinacor(
	Informacao in int,
	Filtro in Varchar2,
...
Begin
	if Informacao=1 then -- Nacionalidade = 1
		OPEN Retorno FOR 
            select CD_NACION as id, DS_NACION as value
            from TSCNACION
			where CD_NACION = TO_NUMBER(Filtro);
	elsif  Informacao=2 then -- EstadoCivil = 2
		OPEN Retorno FOR 
			select CD_EST_CIVIL as id, DS_EST_CIVIL as value 
            from tscestciv
			where CD_EST_CIVIL =  TO_NUMBER(Filtro);
	elsif  Informacao=3 then -- TipoDocumento = 3
		OPEN Retorno FOR 
			select CD_TIPO_DOC as id, DS_TIPO_DOC as value 
            from TSCTIPDOC
			where CD_TIPO_DOC = Filtro;
	elsif  Informacao=4 then -- OrgaoEmissor = 4
		OPEN Retorno FOR
			select CD_ORG_EMIT as id, DS_ORG_EMIT as value 
            from TSCOREMI
			where CD_ORG_EMIT = Filtro;
	elsif  Informacao=5 then -- Profissao = 5
		OPEN Retorno FOR 
			select CD_ATIV as id, DS_ATIV as value
            from tscativ
			where CD_ATIV>100 
			and CD_ATIV =  TO_NUMBER(Filtro);
	elsif  Informacao=6 then -- Banco = 6
		OPEN Retorno FOR 
			select CD_BANCO as id, NM_BANCO as value
            from tscbanco
			where CD_BANCO = Filtro;
	elsif  Informacao=7 then -- Estado = 7
		OPEN Retorno FOR 
			select SG_ESTADO as id, NM_ESTADO as value 
            from TSCESTADO
			where SG_ESTADO = Filtro;
	elsif  Informacao=8 then -- Pais = 8
		OPEN Retorno FOR 
			select SG_PAIS as id, NM_PAIS as value 
            from tscpais
			where SG_PAIS = Filtro;
	elsif  Informacao=9 then -- Assessor = 9
		OPEN Retorno FOR 
			select cd_assessor as id , cd_assessor ||' - '|| nm_assessor as value
            from tscasses
			where cd_assessor = TO_NUMBER(Filtro);
	elsif  Informacao=10 then -- SituacaoLegalRepresentante = 10
		OPEN Retorno FOR 
			select CD_TIPO_FILI as id, DS_TIPO_FILI as value 
            from TSCTIPFIL
			where CD_TIPO_FILI = TO_NUMBER(Filtro);
	elsif  Informacao=11 then -- TipoCliente = 11
		OPEN Retorno FOR 
			select TP_CLIENTE as id,  TP_CLIENTE||' - '|| DS_TIPO_CLIENTE as value 
            from TSCTIPCLI
			where TP_CLIENTE = TO_NUMBER(Filtro);
		...
		OPEN Retorno FOR 
			select TP_CONTA as id, DS_TP_CONTA as value
            from tsctpconta
			where TP_CONTA =  Filtro;
	elsif  Informacao=15 then -- Escolaridade = 15
		OPEN Retorno FOR 
			select CD_ESCOLARIDADE as id, DS_ESCOLARIDADE as value
            from tscescolaridade
			where CD_ESCOLARIDADE =  TO_NUMBER(Filtro);	
	elsif  Informacao = 16 then -- AtividadePF = 16
		OPEN Retorno FOR
		    select CD_ATIV as id, DS_ATIV as value from tscativ where CD_ATIV >= 100 and CD_ATIV  = TO_NUMBER(Filtro);
    elsif  Informacao = 17 then -- AtividadePFePJ = 17
		OPEN Retorno FOR
		    select CD_ATIV as id, DS_ATIV as value from tscativ where CD_ATIV  = TO_NUMBER(Filtro);
	elsif  Informacao = 18 then -- AssessorPadronizado = 18
		OPEN Retorno FOR
		   select cd_assessor as id , nm_assessor as value from tscasses where cd_assessor = TO_NUMBER(Filtro) ;
    end if;
End;
/


create or replace procedure prc_ListaComboSinacor
( Informacao IN  INT
, Retorno    OUT GLOBALPKG.Retorno
)
/*
