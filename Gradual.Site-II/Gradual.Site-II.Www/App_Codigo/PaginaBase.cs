...
namespace Gradual.Site.Www
{
    ...
    public class PaginaBase : System.Web.UI.Page
    {
        ...
        public MensagemResponseStatusEnum EnviarEmail(string pDestinatario, string pAssunto, string pCorpoEmail, eTipoEmailDisparo pTipoEmailDisparo, List<Gradual.OMS.Email.Lib.EmailAnexoInfo> pAnexos = null, List<string> pDestinatariosOcultos = null)
        {
        ...
        public void CarregarClienteEmSessao(string pIdCliente)
        {
            ...
            if (lRetornoSessaoPortal.StatusResposta == MensagemResponseStatusEnum.OK)
            {
                if (lRetornoSessaoPortal.EntidadeCadastro.IdCliente != 0 && lRetornoSessaoPortal.EntidadeCadastro.IdLogin != 0)
                {
                    ...
                    SessaoClienteLogado.CodigoTipoOperacaoCliente = lRetornoSessaoPortal.EntidadeCadastro.CodigoTipoOperacaoCliente;
                    ...
                    this.SessaoClienteLogado.NovoHB = lRetornoSessaoPortal.EntidadeCadastro.NovoHB;
                    this.SessaoClienteLogado.AlterarAssinatura = lRetornoSessaoPortal.EntidadeCadastro.AlterarAssinatura;
                }
                else
                {
        ...
        public bool ValidarSessao()
        {
            ...
            TipoTecladoResponse lTipoTecladoResponse = null;
            
            if(Session["TipoTeclado"] != null)
            {
                lTipoTecladoResponse = (TipoTecladoResponse)Session["TipoTeclado"];
            }

            if (lTipoTecladoResponse != null)
            {
                if (!lTipoTecladoResponse.Teclado.Equals(TipoTeclado.DINAMICO))
                {
                    if (this.SessaoClienteLogado.Passo >= 4)
                    {
                        if (!Request.Url.PathAndQuery.Contains("Seguranca.aspx"))
                        {
                            Session["ExibirMensagemTrocaSenha"] = true;
                            this.Response.Redirect(HostERaizFormat("MinhaConta/Cadastro/Seguranca.aspx"));
                        }
                    }
        ...
        /// <summary>
        /// Metodo paleativo para validação apenas da sessão sem a validação do teclado
        /// </summary>
        /// <returns></returns>
        public bool ValidaApenasSessao()
        {
            if (this.SessaoClienteLogado == null)
            {
                Session["RedirecionamentoPorFaltaDeLogin"] = Request.Url.PathAndQuery;
        ...
        /// <summary>
        /// Lista do sinacor (não estatica) com a opção "SELECIONE" adicionada
        /// </summary>
        /// <param name="pLista">Lista para adicionar o item</param>
        /// <param name="idSelecaoNula">Valor do id do item de seleção nula do dropdown (primeiro item que aparece no dropdown)</param>
        /// <param name="textoSelecaoNula">Texto exibido no item de seleção nula do dropdown (primeiro item que aparece no dropdown)</param>
        /// <param name="pRemoverRepeticoes">Opcional, remove instâncias repetidas de um item</param>
        /// <returns></returns>
        public object ListaComSelecione(List<SinacorListaInfo> pLista, string idSelecaoNula, string textoSelecaoNula, string pRemoverRepeticoes = null)
        {
            var pLista_temp = new List<SinacorListaInfo>();
            pLista.ForEach(item => pLista_temp.Add(new SinacorListaInfo {Id = item.Id, Informacao = item.Informacao, Value = item.Value }));

            if (pLista_temp.Count > 0)
            {

                if (pLista_temp[0].Id != "")
                {
                    pLista_temp.Insert(0, new SinacorListaInfo() { Id = idSelecaoNula, Value = textoSelecaoNula });
                }
                else if (pLista_temp[0].Value == "[SELECIONE]")
                {
                    pLista_temp[0].Value = textoSelecaoNula;
                    pLista_temp[0].Id = idSelecaoNula;
                }
                    ...
                    if (!string.IsNullOrEmpty(pRemoverRepeticoes))
                {
                    ...
                    for (int a = 0; a < pLista_temp.Count; a++)
                    {
                        if (pLista_temp[a].Value == pRemoverRepeticoes)
                        {
                            if (lPrimeiroItem == -1)
                            {
                                //achou o primeiro
                    ...
                    for (int a = lRemover.Count - 1; a >= 0; a--)
                    {
                        pLista_temp.RemoveAt(lRemover[a]);
                    }
                }
            }
