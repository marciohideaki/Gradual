...
//        });
        ...
        var r = Raphael("pnlGrafico_Patrimonio");
...
function SolicitacoesGerenciamentoIPO_DesabilitaFormularioReserva() 
{
    $("#btnConfirmar")      .attr("disabled", "disabled");
    $("#cbo_IPO_Empresa")   .attr("disabled", "disabled");
    $("#cbo_IPO_Modalidade").attr("disabled","disabled");
}

function SolicitacoesGerenciamentoIPO_FecharPopupFormularioReserva() {
    //window.close();
}

function SolicitacoesGerenciamentoIPO_ChangeIPO(pSender) {

    var lIpoSelecionado = $(pSender).val();

    var lObjectModalidades =  $.parseJSON($("#hddListaIpoModalidade").val());

    var lObjectGarantia = $.parseJSON($("#hddListaIpoGarantia").val());

    var lModalidades = lObjectModalidades.ObjetoDeRetorno;

    var lGarantias = lObjectGarantia.ObjetoDeRetorno;

    $("#cbo_IPO_Modalidade").attr('disable', '');

    $("#cbo_IPO_Modalidade").val(lModalidades[lIpoSelecionado]);

    $("#cbo_IPO_Modalidade").attr('disable', 'disable');

    $("#txt_IPO_PercentualGarantias").val(lGarantias[lIpoSelecionado]);
    /*
    for (i = 0; i < lModalidades.length; i++) { 
        
        var lIpo = lModalidades[i].CodigoIPO;
        
        if (lIpoSelecionado == lIpo) {

            var lModalidade = lModalidades[i].Modalidade;

            $("#cbo_IPO_Modalidade").val(lModalidade);
