...
var CONST_ERRO_SOLTECH_USUARIO_INEXISTENTE = "não encontrado para alteração de assinatura digital";
...
///<returns>void</returns>
    ...
    if (pMensagem == "Devido a troca do teclado será necessário que tanto a senha quanto a assinatura eletrônica sejam alteradas!")
    {
        pMensagem = "Devido a última alteração será necessário que suas informações de segurança sejam alteradas!";
    }
...
///<returns>void</returns>
    ...
    else
    {
        if(pResposta.TemErro)
        {
            ...
            {
                if (pResposta.Mensagem.indexOf(CONST_ERRO_SOLTECH_USUARIO_INEXISTENTE) > 0)
                {
...
function GradSite_AtivarInputs(pContainerDosInputs)
{
    ...
    }).keydown(function (ev)
    {
        ...
        if ($("#hidRaiz").val().toLowerCase().indexOf("localhost") == 0)
        {
            var key = ev.keyCode;
