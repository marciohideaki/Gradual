...
function GradSite_Cadastro_PFPasso1_Verificar()
{
    ...
    if(gDadosDeCadastro.ComoConheceu == "CLIENTE")
    {
        gDadosDeCadastro.Assessor = "22";
    }
...
function cboCadastro_PFPasso1_Conheceu_Change(pSender)
{
    ...
    if(cboCadastro_PFPasso1_Conheceu.val() == "ASSESSOR")
    {
        ...
        $("#ContentPlaceHolder1_txtCadastro_PFPasso1_Assessor").val("22");

        $("#lblNomeAssessor").html("0022 - 22 - CLIENTE GRADUAL - SP - MESA COMERCIAL");
    }
    else
    {
        pnlAssessor.hide();
...
function GradSite_Cadastro_PFPasso3_CarregarSuitability()
{
    var lSuit = {
                  ...
                  lSuit.Valido = (lSuit.Resp1 !== undefined) && (lSuit.Resp2 !== undefined) && (lSuit.Resp3 !== undefined) && (lSuit.Resp4 !== undefined) && (lSuit.Resp5 !== undefined) && (lSuit.Resp6 !== undefined) && (lSuit.Resp7 !== undefined) && (lSuit.Resp8 !== undefined);
...
{
    if($("[name='TipoDeAplicacao']:checked").length == 0)
    {
        GradSite_ExibirMensagem("A", "Favor escolher uma das opções");

        return false;
    }

    var lDados = { Acao: "SalvarDesejaAplicar", DesejaAplicar: $("[name='TipoDeAplicacao']:checked").val(), CodFundo: $("#cboFundoParaAplicar").val() };

    $("#lnkTermoFundo").attr( "href", $("#cboFundoParaAplicar option:selected").attr("data-termo"));

    $("#lnkTermoFundo2").attr("href", $("#lnkTermoFundo").attr("href"));

    GradSite_CarregarJsonVerificandoErro("MeuCadastro.aspx", lDados, GradSite_Cadastro_SalvarDesejaAplicar_CallBack);
...
{
    var lCaller = $("#txtCadastro_PFPasso4_SenhaNova").val()

    if (lCaller.length < 8 || lCaller.length > 15) 
    {
        GradSite_ExibirMensagem("A", "Campo deve ter de 8 a 15 caracteres");
    ...
    if (lContNumeros < lContQuantidadeMinimoNumero) 
    {
        ...
        GradSite_ExibirMensagem("A", "Campo deve ter pelo menos um caracter Num&eacuterico");
        return false;
    }
...
function Cadastro_PreencherTeste_P1()
{
    ...
    txtCadastro_PFPasso1_Senha.val("11111111");
    txtCadastro_PFPasso1_SenhaConfirmacao.val("11111111");
    txtCadastro_PFPasso1_AssEletronica.val("11111111");
    txtCadastro_PFPasso1_AssEletronicaConfirmacao.val("11111111");
...
function Cadastro_PreencherTeste_P3(pCompleto)
{
    ...
    if(pCompleto)
    {
        ...
        txtCadastro_PFPasso3_ContasBancarias_AgenciaDigito.val("x");
