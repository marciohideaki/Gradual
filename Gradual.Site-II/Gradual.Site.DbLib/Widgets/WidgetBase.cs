...
{
    public delegate void MensagemDeWidgetEventHandler(object pSender, DateTime pEventDate, string pMensagem);
    ...
    public abstract class WidgetBase
    {
        ...
        public event MensagemDeWidgetEventHandler MensagemDeWidget;

        public void OnMensagemDeWidget(string pMensagem, params object[] pParams)
        {
            if (this.MensagemDeWidget != null)
            {
                MensagemDeWidget(this, DateTime.Now, string.Format(pMensagem, pParams));
            }
        }
        ...
        public string AjustarReferenciasDeLinksNoHTML(string pHTML)
        {
            /*
            string lConteudoHTML = pHTML;

            string lNovoValor;

            Regex lReg = new Regex("(src|href)=\"/.*?\"");

            MatchCollection lMatches = lReg.Matches(lConteudoHTML);

            foreach (Match lMatch in lMatches)
            {
                lNovoValor = lMatch.Value;

                lNovoValor = lNovoValor.Insert(lNovoValor.IndexOf('/'), this.HostERaiz);

                lConteudoHTML = lConteudoHTML.Replace(lMatch.Value, lNovoValor);
            }
            */
            //removido porque estava salvando novamente com o atributo de referência inteiro

            return pHTML;
        }


        public abstract string Renderizar(byte pRenderizandoNaAba);

        #endregion

        public enum TipoWidget
        {
            Titulo, Imagem, Lista, Texto, Tabela, Repetidor, ListaDeDefinicao, Embed, Abas, Acordeon, TextoHTML
        };
