...
namespace Gradual.Spider.Www.App_Codigo.Transporte
{
    ...
    public class Transporte_Ordens
    {
        ...
        public List<Transporte_Ordens> TraduzirList(List<OrderDbInfo> pLista)
        {
            ...
            pLista.ForEach(ordem =>
            {
                ...
                lTrans.ExpireDate   = ordem.ExpireDate.ToString("dd/MM/yyyy HH:mm:ss");
                lTrans.CssOrdensTR  = RetornaCorTr(ordem.OrdStatus.ToString());
                lTrans.Hora         = ordem.TransactTime.ToString("HH:mm:ss");
                lRetorno.Add(lTrans);
            });
