...
namespace Gradual.Spider.LimiteRestricao.DbLib
{
    public class RiscoLimiteDbLib
    {
        ...
        public RiscoListarLimiteAlocadoResponse ConsultarRiscoLimitePorClienteSpider(RiscoListarLimiteAlocadoRequest pParametros)
        {
        ...
        public List<RiscoPermissaoInfo> ListarEntidadeRiscoPermissaoInfo(RiscoPermissaoInfo pParametros)
        {
            ...
            using (DbCommand lDbCommand = lAcessaDados.CreateCommand(CommandType.StoredProcedure, "prc_permissao_lst"))
            {
                ...
                lAcessaDados.AddInParameter(lDbCommand, "@dscr_permissao", DbType.String, pParametros.NomePermissao);
        ...
        public RiscoListarParametrosClienteResponse ListarLimitePorClienteSpider(RiscoListarParametrosClienteRequest pParametro)
        {
            ...
            gLogger.InfoFormat("Entrou na consulta do método de ListarLimitePorClienteSpider do cliente [{0}]", pParametro.CodigoCliente);
            ...
            try
            {
                using (DbCommand lDbCommand = lAcessaDados.CreateCommand(CommandType.StoredProcedure, "prc_cliente_parametro_lst"))
                {
                    ...
                    gLogger.InfoFormat("ListarLimitePorClienteSpider-> antes do ExecuteDbDataTable");

                    var lDataTable = lAcessaDados.ExecuteDbDataTable(lDbCommand);

                    gLogger.InfoFormat("ListarLimitePorClienteSpider-> Depois do ExecuteDbDataTable");

                    lRetorno.ParametrosRiscoCliente = new List<RiscoParametroClienteInfo>();

                    gLogger.InfoFormat("ListarLimitePorClienteSpider-> lDataTable.Rows.Count [{0}] ", lDataTable.Rows.Count);
                    ...
                    if (null != lDataTable && lDataTable.Rows.Count > 0)
                        for (int i = 0; i < lDataTable.Rows.Count; i++)
                        {
                            ...
                            gLogger.InfoFormat("ListarLimitePorClienteSpider-> lRetorno.ParametrosRiscoCliente.Count [{0}] ", lRetorno.ParametrosRiscoCliente.Count);
                        }
                }

                lRetorno.StatusResposta = MensagemResponseStatusEnum.OK;

                gLogger.InfoFormat("ListarLimitePorClienteSpider-> lRetorno.StatusResposta [{0}] ", lRetorno.StatusResposta);
            }
            catch (Exception ex)
            {
                gLogger.Error(ex);
        ...
        public RiscoSalvarParametroClienteResponse SalvarPermissoesRiscoAssociadasSpider(RiscoSalvarPermissoesAssociadasRequest pRequest)
        {
