...
namespace QuickFix.Fields
{
    ...
    public abstract class FieldBase<T> : IField
    {
        ...
        public override int getLength()
        {
            if (_changed)
                makeStringFields();
            //return System.Text.Encoding.GetEncoding(1252).GetByteCount(_stringField) + 1; // +1 for SOH
            return Values.ENCODING.GetByteCount(_stringField) + 1; // +1 for SOH
        }
        ...
        public override int getTotal()
        {
            ...
            int sum = 0;
            //byte[] array = System.Text.Encoding.GetEncoding(1252).GetBytes(_stringField);

            byte[] array = Values.ENCODING.GetBytes(_stringField);
