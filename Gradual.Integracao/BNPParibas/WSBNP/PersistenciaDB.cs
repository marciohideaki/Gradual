    ...
    public class PersistenciaDB
    {
        ...
        public Dictionary<string, TraderInfo> CarregarTraderInfo()
        {
            ...
            if (traderConfig != null)
            {
                ...
                {
                    logger.Debug("Loading TraderInfo for [" + info.CpfCnpj + "]");
        ...
        public Dictionary<string, PLDInfo> CarregarPLDInfo()
        {

            Dictionary<string, PLDInfo> ret = new Dictionary<string, PLDInfo>();

            TraderConfig traderConfig = Gradual.OMS.Library.GerenciadorConfig.ReceberConfig<TraderConfig>();

            if (traderConfig != null)
            {
                foreach (PLDInfo info in traderConfig.PLDList)
                {
                    logger.Debug("Loading PLDInfo for [" + info.AccountGradual + "]");
                    ret.Add(info.AccountGradual, info);
                }
            }
        ...
        public Dictionary<long, Trade> BuscarTrades(string datanegos, long lastSequence, string clientIdList)
        {
            ...
            query += "      C.NR_SEQORD as NR_SEQORD,";
            query += "      N.DT_STAMP,";
            query += "      C.NR_NEGOCIO,";
            query += "      C.TP_REGISTRO,";
            query += "      C.DT_NEGOCIO,";
            ...
            query += "      left join TMFSERIE T ON ( T.CD_COMMOD = C.CD_COMMOD ";
            query += "        AND  T.CD_SERIE = C.CD_SERIE  ";
            query += "        AND T.CD_MERCAD = C.CD_MERCAD )";
            query += "      WHERE";
            query += "       C.NR_SEQORD > " + lastSequence + "  ";
        ...
        private Trade _Sinacor2Trade(OracleDataReader odr)
        {
            ...
            Dictionary<string, PLDInfo> pldList = this.CarregarPLDInfo();
            ...
            try
            {
                ...
                trade.Sequencial = OracleConvert.GetLong("NR_SEQCOMI", odr);
                trade.NumOrdem = OracleConvert.GetLong("NR_SEQORD", odr);
                trade.NumNegocio = OracleConvert.GetLong("NR_NEGOCIO", odr);
                ...
                trade.Commod = OracleConvert.GetString("CD_COMMOD", odr).Trim();
                trade.Serie = OracleConvert.GetString("CD_SERIE", odr).Trim();
                trade.Mercado = OracleConvert.GetString("CD_MERCAD", odr).Trim();
                trade.NomeAtivo = trade.Commod + trade.Mercado.Substring(0, 1) + trade.Serie;
                ...
                {
                    logger.Error("Trader ID [" + traderid + "] nao encontrado. Verificar .config. Skipping");
                    return null;
                }
                ...
                //}
                //trade.NumContaCliente = traderInfo.TraderIDBNP;

                logger.Debug("Buscando PLD info [" + trade.NumContaCliente + "]");
                if (!pldList.ContainsKey(trade.NumContaCliente))
                {
                    logger.Error("Informacoes de PLD para conta [" + trade.NumContaCliente + "] nao encontradas. Verificar .config. Skiping");
                    return null;
                }

                PLDInfo pldInfo = pldList[trade.NumContaCliente];
                ...
                trade.CodCorretoraBMFDestino = Convert.ToInt64(pldInfo.AccountPLD);
                trade.NomeCorretoraBMFDestino = pldInfo.NomePLD;

                trade.DataVencimento = OracleConvert.GetDateTime("DT_VENC", odr);
                trade.TraderID = OracleConvert.GetString("CD_CPFCGC_EMIT", odr);
                trade.TraderID = traderInfo.TraderIDBNP;
                ...
                trade.ValorFuturoCurto = Convert.ToDecimal(OracleConvert.GetDouble("VL_FUT_CURTO", odr)).ToString();
                trade.ValorFuturoLongo = Convert.ToDecimal(OracleConvert.GetDouble("VL_FUT_LONGO", odr)).ToString();
                trade.LeilaoTermo = "N";
                trade.Isin = OracleConvert.GetString("CD_ISIN", odr);
        ...
        private Dictionary<long, Trade> _Sinacor2TradeList(OracleDataReader datareader)
        {
            ...
            while (datareader.Read())
            {
                ...
                if (trade != null)
                {
                    if ( !ret.ContainsKey(trade.NumOrdem+trade.NumNegocio) )
                        ret.Add(trade.NumOrdem + trade.NumNegocio, trade);
                }
            }
