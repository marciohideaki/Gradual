...
namespace Gradual.FIDC.Adm.DbLib.App_Codigo
{
    public static class Extensions
    {
        ...
        {
            if (pObject.DBToString().ToUpper() == "S") return true;

            if (pObject.DBToString().ToUpper() == "N") return false;

            bool lRetorno = default(bool);
        ...
        public static DateTime DBToDateTime(this object pObject)
        {
            ...
            if (DateTime.TryParseExact(pObject.ToString(), new string[] { "dd/MM/yyyy", "MM/dd/yyyy", "dd/MM/yyyy HH:mm:ss", "MM/dd/yyyy HH:mm:ss", "M/d/yyyy h:mm:ss tt", "d/MM/yyyy HH:mm:ss", "d/MM/yyyy" }, System.Globalization.CultureInfo.InvariantCulture, DateTimeStyles.None, out lReturn))
            {
                return lReturn;
            }
            else if (DateTime.TryParse(pObject.DBToString(), out lReturn))
            {
                return lReturn;
            }
            else
        ...
        public static string ToCpfCnpjString(this string pNumero)
        {
            ...
            if (pNumero.Length <= 11)
                lRetorno = pNumero.PadLeft(11, '0');
            else
                lRetorno = pNumero.PadLeft(14, '0');

            return lRetorno;
        }

        public static string ToStringSemAcentos(this string pString)
        {
            string lRetorno = pString;

            lRetorno = lRetorno.Replace('á', 'a');
            lRetorno = lRetorno.Replace('â', 'a');
            lRetorno = lRetorno.Replace('à', 'a');
            lRetorno = lRetorno.Replace('ã', 'a');

            lRetorno = lRetorno.Replace('é', 'e');
            lRetorno = lRetorno.Replace('ê', 'e');
            lRetorno = lRetorno.Replace('è', 'e');

            lRetorno = lRetorno.Replace('í', 'i');
            lRetorno = lRetorno.Replace('î', 'i');
            lRetorno = lRetorno.Replace('ì', 'i');

            lRetorno = lRetorno.Replace('ó', 'o');
            lRetorno = lRetorno.Replace('ô', 'o');
            lRetorno = lRetorno.Replace('ò', 'o');
            lRetorno = lRetorno.Replace('õ', 'o');

            lRetorno = lRetorno.Replace('ú', 'u');
            lRetorno = lRetorno.Replace('û', 'u');
            lRetorno = lRetorno.Replace('ù', 'u');
            lRetorno = lRetorno.Replace('ü', 'u');

            lRetorno = lRetorno.Replace('ç', 'c');
        ...
        /// <returns></returns>
        public static int ToCodigoClienteDigito(this object pObject)
        {
            int lDigito = 0;
