			...
			for (int minute = (lastMinute == 59 ? 0 : lastMinute + 1); minute <= now.Minute; minute++)
			{
				foreach (ArrayList entry in crontab)
				{
						...
						contains (entry[4], now.Minute))
					{
                        ...
                        string file2down = (String) entry[5];
                        
                        logger.Info("Executando [" + file2down + "]");

                        switch (file2down)
                        {
                            case "XMLSaldosAberturaD0":
                                {
                                    ThreadPool.QueueUserWorkItem(new WaitCallback(
                                     delegate(object required)
                                     {
                                         ...
                                         downloader._download_SaldosCotistasD0();
                                     }));
                                }
                                break;

                            default:
                                logger.Info("Nao existe comando para o arquivo [" + file2down+ "]");
                                break;
                        }
					}
				}
